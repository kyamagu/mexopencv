/**
 * @file MergeDebevec_.cpp
 * @brief mex interface for cv::MergeDebevec
 * @ingroup photo
 * @author Amro
 * @date 2015
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (method == "new") {
        nargchk(nrhs==2 && nlhs<=1);
        obj_[++last_id] = createMergeDebevec();
        plhs[0] = MxArray(last_id);
        return;
    }

    // Big operation switch
    Ptr<MergeDebevec> obj = obj_[id];
    if (method == "delete") {
        ...
        obj_[id] = (loadFromString ?
            Algorithm::loadFromString<MergeDebevec>(rhs[2].toString(), objname) :
            Algorithm::load<MergeDebevec>(rhs[2].toString(), objname));
        */
        ///*
        // HACK: workaround for missing MergeDebevec::create()

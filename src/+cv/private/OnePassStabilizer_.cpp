 ...
 * @brief mex interface for cv::videostab::OnePassStabilizer
 * @ingroup videostab
 * @author Amro
 * @date 2016
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (method == "new") {
        nargchk(nrhs==2 && nlhs<=1);
        obj_[++last_id] = makePtr<OnePassStabilizer>();
    ...
    Ptr<OnePassStabilizer> obj = obj_[id];
    if (obj.empty())
    ...
    else if (method == "setMotionFilter") {
        nargchk(nrhs>=3 && nlhs==0);
        Ptr<MotionFilterBase> p = createMotionFilterBase(
            rhs[2].toString(), rhs.begin() + 3, rhs.end());
        obj->setMotionFilter(p);
    }
    ...
    else if (method == "getMotionFilter") {
        nargchk(nrhs==2 && nlhs<=1);
        Ptr<MotionFilterBase> p = obj->motionFilter();
        plhs[0] = toStruct(p);
    }
    else if (method == "get") {
        nargchk(nrhs==3 && nlhs<=1);

...
{
public:
    MatlabFunction(int num_dims, const string &func, const string &grad_func)
    : dims(num_dims), fun_name(func), grad_fun_name(grad_func)
    {}
    ...
    void getGradient(const double* x, double* grad) /*const*/
    {
        ...
        mxArray *lhs, *rhs[2];
        rhs[0] = mxCreateString(grad_fun_name.c_str());
        rhs[1] = mxCreateDoubleMatrix(1, dims, mxREAL);
        memcpy(mxGetPr(rhs[1]), x, sizeof(double)*dims);

        // evaluate specified function: grad = feval("grad_fun_name", x)
        if (mexCallMATLAB(1, &lhs, 2, rhs, "feval") == 0)
            memcpy(grad, mxGetPr(lhs), sizeof(double)*mxGetNumberOfElements(lhs));
    ...
    static Ptr<MatlabFunction> create(const MxArray &s)
    {
        ...
        return makePtr<MatlabFunction>(s.at("dims").toInt(),
            s.at("fun").toString(), s.at("gradfun").toString());
    }

private:
    int dims;
    string fun_name, grad_fun_name;
};
}

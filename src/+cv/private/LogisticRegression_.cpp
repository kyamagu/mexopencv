...
const ConstMap<std::string,int> TrainingMethodType = ConstMap<std::string,int>
    ("Batch",     LogisticRegression::BATCH)
    ("MiniBatch", LogisticRegression::MINI_BATCH);

/// Option values for Inverse Training methods
const ConstMap<int,std::string> InvTrainingMethodType = ConstMap<int,std::string>
    (LogisticRegression::BATCH,      "Batch")
    (LogisticRegression::MINI_BATCH, "MiniBatch");

/// Option values for Regularization kinds
const ConstMap<std::string,int> RegularizationType = ConstMap<std::string,int>
    ("Disable", LogisticRegression::REG_DISABLE) //!< Regularization disabled
    ("L1",      LogisticRegression::REG_L1)      //!< L1 norm
    ("L2",      LogisticRegression::REG_L2);     //!< L2 norm

/// Option values for Inverse Regularization kinds
const ConstMap<int,std::string> InvRegularizationType = ConstMap<int,std::string>
    (LogisticRegression::REG_DISABLE, "Disable")
    (LogisticRegression::REG_L1,      "L1")
    (LogisticRegression::REG_L2,      "L2");
}
                  ...
                  int nrhs, const mxArray *prhs[] )
{
    ...
    if (method == "delete") {
        ...
        if (loadFromString)
            obj = Algorithm::loadFromString<LogisticRegression>(rhs[2].toString(), objname);
        else
            obj = Algorithm::load<LogisticRegression>(rhs[2].toString(), objname);
    }
    else if (method == "save") {
        if (nrhs!=3 || nlhs!=0)
            mexErrMsgIdAndTxt("mexopencv:error","Wrong number of arguments");

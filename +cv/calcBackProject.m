...
%   array of the same row/column size and depth as `images` (if numeric) or
%   `images{1}` (if `images` is a cell array). Out of `uint8`, `uint16`, or
%   `single` classes. Pay attention to the data type of the back projection
%   array, where integer types are clamped to the maximum `intmax` if a value
%   exceeds the largest possible integer of the type.
%
% ## Options
...
%   `0:sum(cellfun(@(im)size(im,3), images))-1` when input `images` is a cell
%   array, and `0:(size(images,3)-1)` when input `images` is a numeric array.

...
%   search zone over which the summation in the formula below is not done. It
%   is used sometimes to avoid possible singularities of the autocorrelation
%   matrix. The value of `[-1,-1]` indicates that there is no such a size.
%   default [-1,-1].
...
%   `struct('type','Count+EPS', 'maxCount',50, 'epsilon',0.001)`. Struct with

...
%		floating point matrix. If you want to apply different kernels to
%		different channels, split the image into separate color planes and
%		process them individually.
%  Output:
%    result:
%       Destination image of the same size and the same number of channels as
%		src.
%  Options:
%    'Anchor':
%		Anchor of the kernel that indicates the relative position of a filtered
%		point within the kernel. The anchor should lie within the kernel. The
%		special default value (-1,-1) means that the anchor is at the kernel
%		center.
%    'BorderType':
%       Pixel extrapolation method. Default 'Default'
%

...
%   retained in the undistorted image). See cv.stereoRectify for details.
%   default 0.8
% * __NewImageSize__ Image size after rectification `[w,h]`. By default, it is
%   set to `imageSize`.
% * __CenterPrincipalPoint__ Optional flag that indicates whether in the new
...
%   image (determined by `Alpha`) to the corrected image. default false.
%

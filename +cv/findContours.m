...
%   You can use cv.compare, cv.inRange, cv.threshold, cv.adaptiveThreshold,
%   cv.Canny, and others to create a binary image out of a grayscale or color
%   one. If mode equals to `CComp` or `FloodFill`, the input can also be a
%   32-bit integer image of labels (`int32` class).
%
% ## Output
...
%   each i-th contour, `contours{i}`, the elements `hierarchy{i}(1)`,
...
%   same hierarchical level, the first child contour and the parent contour,
%   respectively. If for the i-th contour there are no next, previous, parent,
%   or nested contours, the corresponding elements of `hierarchy{i}` will be
%   negative. A cell array of 4-element integer vectors of the form
...
%   * __Tree__ retrieves all of the contours and reconstructs a full hierarchy
%     of nested contours
...
%     chain approximation algorithm [TehChin89] (1-curvature or k-cosine
%     curvature).
...
%   should be analyzed in the whole image context. default [0,0]
%

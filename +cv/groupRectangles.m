...
%   input `rects` (either Nx4 numeric array or cell array). Grouped rectangles
%   are the average of all rectangles in that cluster.
% * __weights__ optional output of filled/updated weights, with same length as
%   output `rects`. See `Weights` option. Corresponding grouped weights are
%   the maximum weights of all rectangles in that cluster.
...
%   threshold is used in a group of rectangles to decide whether to retain it
%   or not. If less than or equal to zero, no grouping is performed. default 1
%   (i.e only groups with two or more rectangles are kept).
% * __EPS__ Relative difference between sides of the rectangles to merge them
%   into a group. default 0.2

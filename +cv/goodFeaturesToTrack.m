...
%   `MaxCorners <= 0` implies that no limit on the maximum is set and all
...
%   quality measure, which is the minimal eigenvalue (see cv.cornerMinEigenVal)
%   or the Harris function response (see cv.cornerHarris). The corners with
%   the quality measure less than the product are rejected. For example, if
%   the best corner has the quality measure = 1500, and the
%   `QualityLevel=0.01`, then all the corners with the quality measure less
%   than 15 are rejected. default 0.01
...
%   specifies the region in which the corners are detected. It is not set by
%   default.

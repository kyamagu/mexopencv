...
%   `[w,x1,x2,...,xn]`. The matrix is allowed to have a single column (weights
%   only) if the user-defined `Cost` matrix is used. The weights must be
...
%   either `signature1` or `signature2`. The weights must be non-negative and
...
%   `flow(i,j)` is a flow from i-th point of `signature1` to j-th point of
%   `signature2`.
%
% ## Options
...
%   * __User__ User-defined distance, means that a pre-calculated cost matrix
%     is used. Should be used when `Cost` is set.
% * __Cost__ User-defined `size1-by-size2` cost matrix. Also, if a cost matrix
%   is used, output lower boundary `lowerBound` cannot be calculated because
%   it needs a metric function. Not set by default
...
%   centers. The lower boundary may not be calculated if the user-defined cost
%   matrix is used, the total weights of point configurations are not equal,
%   or if the signatures consist of weights only (the signature matrices have
%   a single column). If the calculated distance between mass centers is
%   greater or equal to `LowerBound` (it means that the signatures are far
%   enough), the function does not calculate EMD. In any case `LowerBound` is

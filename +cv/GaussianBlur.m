...
%
% ## Options
% * __KSize__ Gaussian kernel size. `KSize(1)` and `KSize(2)` can differ but
%   they both must be positive and odd. Or, they can be zeros and they are
%   computed from `SigmaX` and `SigmaY`. default [5,5]
...
%   is zero, it is set to be equal to `SigmaX`. If both sigmas are zeros, they
%   are computed from `KSize(2)` and `KSize(1)`, respectively (see
...
%   to specify all of `KSize`, `SigmaX`, and `SigmaY`. default 0

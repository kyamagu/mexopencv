classdef HOGDescriptor < handle
    ...
    properties (Dependent)
        ...
        % Block size
        BlockSize
        % Block stride of a grid
        BlockStride
        % Cell size of a grid
        CellSize
        % Number of bins
        NBins
        % Derivative of aperture
        DerivAperture
        % Window sigma
        WinSigma
        % Histogram normalization method
        HistogramNormType
        % L2 Hysterisis threshold
        L2HysThreshold
        % Gamma correction
        GammaCorrection
        % Number of levels
    ...
    methods
        function this = HOGDescriptor(varargin)
            ...
            %   load from (XML or YAML). This is handled by the load method.
            %
            % ## Output
            % * __hog__ HOG descriptor object.
            %
            % ## Options
            ...
            %   positive/negative directions count as the same histogram bin.
            %   default false
            %
        ...
        function [grad, angleOfs] = computeGradient(this, im, varargin)
            ...
            %   the range `[0,NBins-1]` (2-channel 8-bit matrix of same size
        ...
        function [rcts, weights] = detectMultiScale(this, im, varargin)
            ...
            %   threshold. When detected, some objects can be covered by many
            %   rectangles. 0 means not to perform grouping. See
            %   cv.groupRectangles. default 2.0
            % * __UseMeanshiftGrouping__ Flag to use meanshift grouping or the
            %   default grouping based on merging overlapped rectangles. When
            %   false, cv.HOGDescriptor.groupRectangles is performed and the
            %   value of `FinalThreshold` is used for `GroupThreshold`. When
        ...
        function [pts, confidences] = detectROI(this, im, locations, varargin)
            ...
            %   which detector is executed. These are top-left corner of
            ...
            %   `locations` at which objects where actually detected.
        ...
        function [rects, weights] = groupRectangles(this, rects, weights, varargin)
            ...
            %   is represented as a 4-element vector `{[x,y,w,h], ...}`, or a
            %   numeric Nx4/Nx1x4/1xNx4 array.
            ...
            %   group minus 1. The threshold is used on a group of rectangles
            %   to decide whether to retain it or not. If less than or equal
            %   to zero, no grouping is performed. default 1 (i.e only groups
            %   with two or more rectangles are kept).
            %

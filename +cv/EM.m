classdef EM < handle
    ...
    properties (Dependent)
        ...
        %   parameter `mu_k` to be estimated for each matrix. The option may
        ...
        %   The number of free parameters is `d` for each matrix. This is most
        %   commonly used option yielding good estimation results.
    ...
    methods
        function this = EM(varargin)
            ...
            %
            % ## Input
            % * __filename__ Name of XML or YML file, containing previously
            %   saved model
            %
    ...
    methods
        ...
        function status = train(this, samples, varargin)
            ...
            %   of which is a sample (see the `Layout` option). Internally the
            %   computations are performed in `double` precision type.
        ...
        function [results,f] = predict(this, samples, varargin)
            ...
            %   sample. Otherwise the result for the first sample is returned.
            %
            % ## Options
    ...
    methods
        ...
        function [logLikelihoods, labels, probs] = trainEM(this, samples)
            ...
            %   which is a sample. If the matrix does not have `double` type
            ...
            %   sample. It has `nsamples-by-ClustersNumber` size and `double`
            %   type.
            %
        ...
        function [logLikelihoods, labels, probs] = trainE(this, samples, means0, varargin)
            ...
            %   matrices of such type for the further computing.
        ...
        function [logLikelihoods, labels, probs] = trainM(this, samples, probs0)
            ...
            %   floating-point matrix of `nsamples-by-ClustersNumber` size.
            %
            % ## Output
            % * __logLikelihoods__ See the cv.EM.trainEM method.
            % * __labels__ See the cv.EM.trainEM method.
        ...
        function [logLikelihoods, labels, probs] = predict2(this, samples)
            ...
            %   one-channel matrix of size `nsamples-by-dims` with each row
            %   representing one sample.
            %
            % ## Output
            % * __logLikelihoods__ Output vector that contains a likelihood
            %   logarithm value for each sample. It has `nsamples-by-1` size
            %   and `double` type.
            ...
            %   probabilities of each mixture component given the sample. It
            %   has `nsamples-by-ClustersNumber` size and `double` type.
            %

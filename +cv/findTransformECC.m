...
%   final `warpMatrix` in order to provide an image similar to `templateImage`,
%   same type as `templateImage`.
%
% ## Output
...
%   * __Translation__ sets a translational motion model; `warpMatrix` is 2x3
%     with the first 2x2 part being the unity matrix and the rest two
%     parameters being estimated.
%   * __Euclidean__ sets a Euclidean (rigid) transformation as motion model;
%     three parameters are estimated; `warpMatrix` is 2x3.
%   * __Affine__ sets an affine motion model (DEFAULT); six parameters are
%     estimated; `warpMatrix` is 2x3.
%   * __Homography__ sets a homography as a motion model; eight parameters are
%     estimated; `warpMatrix` is 3x3.
% * __Criteria__ parameter, specifying the termination criteria of the ECC
%   algorithm; `Criteria.epsilon` defines the threshold of the increment in
...
%   Not set by default.
% * __InputWarp__ Initial estimate for `warpMatrix`. See the notes below.
%   Default `eye(2,3)` or `eye(3,3)` depending on motion type.
%

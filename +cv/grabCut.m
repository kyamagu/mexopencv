...
%   It will automatically create the `mask`. The pixels outside of the ROI are
%   marked as "obvious background" with label 0, and label 3 for foreground
%   (see `mask`). Using this variant will set `Mode` to 'InitWithRect'.
...
%   * __InitWithRect__ The function initializes the state and the mask using
%     the provided rectangle. After that it runs `IterCount` iterations of the
%     algorithm. This should only be used with the variant of the function
%     that takes `rect` as input.
%   * __InitWithMask__ The function initializes the state using the provided
%     `mask`. Then, all the pixels outside of the ROI are automatically
%     initialized as background with label 0. This should only be used with
%     the variant of the function that takes `mask` as input.

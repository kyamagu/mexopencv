...
% * __R12__ Rotation matrix between the coordinate systems of the first and
%   the second cameras, 3x3/3x1 (see cv.Rodrigues)
% * __T12__ Translation vector between coordinate systems of the first and the
%   second cameras, 3x1.
% * __R13__ Rotation matrix between the coordinate systems of the first and
%   the third cameras, 3x3/3x1 (see cv.Rodrigues)
...
%   * __P3__ 3x4 projection matrix in the new (rectified) coordinate systems
...
%     pixels are valid `[x,y,w,h]`. If `Alpha=0`, the ROIs cover the whole
%     images. Otherwise, they are likely to be smaller.
%   * __ratio__ disparity ratio, floating-point numeric scalar.
%
% ## Options
% * __ImgPoints1__, __ImgPoints3__ Optional cell arrays of 2D points
%   `{[x,y], ...}`, used to adjust the third matrix `P3`. Not set by default.
...
%   and 1. `Alpha=0` means that the rectified images are zoomed and shifted so
...
%   all the pixels from the original images from the cameras are retained in
%   the rectified images (no source image pixels are lost). Obviously, any
%   intermediate value yields an intermediate result between those two extreme
%   cases. default -1

classdef SVM < handle
    ...
    properties (Dependent)
        ...
        %   classification with possible imperfect separation. Parameter `Nu`
        ...
        %   training data are from the same class, SVM builds a boundary that
        %   separates the class from the rest of the feature space.
        % * **EPS_SVR** P-Support Vector Regression. The distance between
        %   feature vectors from the training set and the fitting hyper-plane
        %   must be less than `P`. For outliers the penalty multiplier `C` is
        %   used.
    ...
    methods
        ...
        function b = isClassifier(this)
            ...
            %   `NU_SVC`, or `ONE_CLASS`), false if the model is a regressor
            %   (`EPS_SVR`, `NU_SVR`).
            %
    ...
    methods
        function status = trainAuto(this, samples, responses, varargin)
            ...
            %   divided into `KFold` subsets. One subset is used to test the
            %   model, the others form the train set. So, the SVM algorithm is
            %   executed `KFold` times. default 10
            % * __Balanced__ If true and the problem is 2-class classification
            %   then the method creates more balanced cross-validation subsets
            %   that is proportions between classes in subsets are close to
            %   such proportion in the whole train dataset. default false
            ...
            %   A structure that represents the logarithmic grid range of SVM
            %   parameters. It is used for optimizing model accuracy by
            %   varying model parameters, the accuracy estimate being computed
            %   by cross-validation. It accepts a struct having the fields
            %   below. It also accepts a 3-element vector in which each
            %   parameter is specified in the same order as the supported
            %   struct:
            %
            ...
            %   model parameter values: `minVal * logStep.^(0:n)`, where `n`
            %   is the maximal index satisfying `minVal*logStep^n < maxVal`.
            %   The grid is logarithmic, so `logStep` must always be greater
            %   than 1. Defaults are:
            %
        ...
        function [alpha,svidx,rho] = getDecisionFunction(this, index)
            ...
            %
            % ## Input
            % * __index__ the index of the decision function (0-based). If the
            %   problem solved is regression, 1-class or 2-class
            %   classification, then there will be just one decision function
            %   and the index should always be 0. Otherwise, in the case of
            %   N-class classification, there will be `N(N-1)/2` decision
            %   functions.
            %
            % ## Output
            ...
            %   retrieved by cv.SVM.getSupportVectors. In the case of linear
            %   SVM each decision function consists of a single "compressed"
            %   support vector.

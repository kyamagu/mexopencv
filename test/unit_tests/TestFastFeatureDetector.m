classdef TestFastFeatureDetector
    ...
    methods (Static)
        function test_detect_img
            ...
            kpts = obj.detect(TestFastFeatureDetector.img);
        ...
        function test_detect_imgset
            imgs = {TestFastFeatureDetector.img, TestFastFeatureDetector.img};
        ...
        function test_detect_mask
            mask = zeros(size(TestFastFeatureDetector.img), 'uint8');
            mask(:,1:end/2) = 255;  % only search left half of the image
            obj = cv.FastFeatureDetector();
            kpts = obj.detect(TestFastFeatureDetector.img, 'Mask',mask);
            validateattributes(kpts, {'struct'}, {'vector'});
            assert(all(ismember(TestFastFeatureDetector.kfields, fieldnames(kpts))));
            xy = cat(1, kpts.pt);
            assert(all(xy(:,1) <= ceil(size(TestFastFeatureDetector.img,2)/2)));
        end

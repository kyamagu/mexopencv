...
files_l = dir(fullfile(cv_root(),'test','left*.jpg'));
files_r = dir(fullfile(cv_root(),'test','right*.jpg'));
...
for i = 1:numel(files_l)
    im_l = imread(fullfile(cv_root(),'test',files_l(i).name));
    im_r = imread(fullfile(cv_root(),'test',files_r(i).name));

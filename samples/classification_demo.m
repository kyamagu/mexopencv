...
% Try trees
trees = {cv.DTree, cv.Boost, cv.RTrees, cv.ERTrees};
for i=1:numel(trees)
    trees_demo(trees{i}, X, Y, test_idx);
end

% Try Others
classifiers = {cv.NormalBayesClassifier, cv.KNearest, cv.SVM};
...
function trees_demo(classifier, X, Y, test_idx)
%TREES_DEMO

fprintf('=== %s ===\n',class(classifier));
tic;
% 'VarType' option specifies a classification problem in trees
classifier.train(X(~test_idx,:),Y(~test_idx),'VarType','Categorical');
fprintf('Training time %f seconds\n',toc);
tic;

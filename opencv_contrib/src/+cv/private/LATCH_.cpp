                  ...
                  int nrhs, const mxArray *prhs[] )
{
    ...
    if (method == "new") {
        if (nrhs!=2 || nlhs>1)
            mexErrMsgIdAndTxt("mexopencv:error","Wrong number of arguments");
        obj_[++last_id] = createLATCH(rhs.begin() + 2, rhs.end());
        plhs[0] = MxArray(last_id);
        return;
    }

    // Big operation switch
    Ptr<LATCH> obj = obj_[id];
    if (method == "delete") {
        if (nrhs!=2 || nlhs!=0)
        ...
        obj_[id] = (loadFromString ?
            Algorithm::loadFromString<LATCH>(rhs[2].toString(), objname) :
            Algorithm::load<LATCH>(rhs[2].toString(), objname));
    }
    else if (method == "save") {
        if (nrhs!=3 || nlhs!=0)
            mexErrMsgIdAndTxt("mexopencv:error","Wrong number of arguments");
        ...
        if (rhs[2].isNumeric()) {  // first variant that accepts an image
            ...
            vector<vector<KeyPoint> > keypoints(rhs[3].toVector<vector<KeyPoint> >());  //TODO: check!!!

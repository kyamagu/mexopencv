/**
 * @file SuperpixelLSC_.cpp
 * @brief mex interface for cv::ximgproc::SuperpixelLSC
 * @ingroup ximgproc
 * @author Amro
 * @date 2016
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (method == "new") {
        ...
        for (int i=3; i<nrhs; i+=2) {
            ...
            if (key == "RegionSize")
                ...
                ratio = rhs[i+1].toFloat();
            else
                mexErrMsgIdAndTxt("mexopencv:error",
                    "Unrecognized option %s", key.c_str());
        }
        Mat image(rhs[2].toMat());
        obj_[++last_id] = createSuperpixelLSC(image, region_size, ratio);
    ...
    if (method == "delete") {
        ...
        obj_[id] = (loadFromString ?
            Algorithm::loadFromString<SuperpixelLSC>(rhs[2].toString(), objname) :
            Algorithm::load<SuperpixelLSC>(rhs[2].toString(), objname));
        */
        ///*
        // HACK: workaround for missing SuperpixelLSC::create()

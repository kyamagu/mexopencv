...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (method == "new") {
        nargchk(nrhs>=2 && nlhs<=1);
        obj_[++last_id] = createStarDetector(rhs.begin() + 2, rhs.end());
        plhs[0] = MxArray(last_id);
        return;
    }

    // Big operation switch
    Ptr<StarDetector> obj = obj_[id];
    if (method == "delete") {
        ...
        obj_[id] = (loadFromString ?
            Algorithm::loadFromString<StarDetector>(rhs[2].toString(), objname) :
            Algorithm::load<StarDetector>(rhs[2].toString(), objname));
    }
    else if (method == "save") {
        ...
        if (rhs[2].isNumeric()) {  // first variant that accepts an image
            ...
            Mat image(rhs[2].toMat(rhs[2].isUint16() ? CV_16U : CV_8U));
            ...
            vector<Mat> images;
            {
                ...
                for (vector<MxArray>::const_iterator it = arr.begin(); it != arr.end(); ++it)
                    images.push_back(it->toMat(it->isUint16() ? CV_16U : CV_8U));
            }

 ...
 * @brief mex interface for cv::bgsegm::BackgroundSubtractorGSOC
 * @ingroup bgsegm
 * @author Amro
 * @date 2018
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (method == "new") {
        ...
        for (int i=2; i<nrhs; i+=2) {
            ...
            else if (key == "ReplaceRate")
                replaceRate = rhs[i+1].toFloat();
            else if (key == "PropagationRate")
                propagationRate = rhs[i+1].toFloat();
            else if (key == "HitsThreshold")
                hitsThreshold = rhs[i+1].toInt();
            else if (key == "Alpha")
                alpha = rhs[i+1].toFloat();
            else if (key == "Beta")
                beta = rhs[i+1].toFloat();
            else if (key == "BlinkingSupressionDecay")
                blinkingSupressionDecay = rhs[i+1].toFloat();
            else if (key == "BlinkingSupressionMultiplier")
                blinkingSupressionMultiplier = rhs[i+1].toFloat();
        ...
        obj_[++last_id] = createBackgroundSubtractorGSOC(mc, nSamples,
            replaceRate, propagationRate, hitsThreshold, alpha, beta,
            blinkingSupressionDecay, blinkingSupressionMultiplier,
    ...
    Ptr<BackgroundSubtractorGSOC> obj = obj_[id];
    if (obj.empty())
    ...
    if (method == "delete") {
        ...
        obj_[id] = (loadFromString ?
            Algorithm::loadFromString<BackgroundSubtractorGSOC>(rhs[2].toString(), objname) :
            Algorithm::load<BackgroundSubtractorGSOC>(rhs[2].toString(), objname));
        */
        ///*
        // HACK: workaround for missing BackgroundSubtractorGSOC::create()

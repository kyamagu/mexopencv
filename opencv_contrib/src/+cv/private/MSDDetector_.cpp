/**
 * @file MSDDetector_.cpp
 * @brief mex interface for cv::xfeatures2d::MSDDetector
 * @ingroup xfeatures2d
 * @author Amro
 * @date 2016
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (method == "new") {
        nargchk(nrhs>=2 && nlhs<=1);
        obj_[++last_id] = createMSDDetector(rhs.begin() + 2, rhs.end());
    ...
    if (method == "delete") {
        ...
        obj_[id] = (loadFromString ?
            Algorithm::loadFromString<MSDDetector>(rhs[2].toString(), objname) :
            Algorithm::load<MSDDetector>(rhs[2].toString(), objname));
    }
    else if (method == "save") {

/**
 * @file HarrisLaplaceFeatureDetector_.cpp
 * @brief mex interface for cv::xfeatures2d::HarrisLaplaceFeatureDetector
 * @ingroup xfeatures2d
 * @author Amro
 * @date 2017
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (method == "new") {
        nargchk(nrhs>=2 && nlhs<=1);
        obj_[++last_id] = createHarrisLaplaceFeatureDetector(
            rhs.begin() + 2, rhs.end());
    ...
    Ptr<HarrisLaplaceFeatureDetector> obj = obj_[id];
    if (obj.empty())
    ...
    if (method == "delete") {
        ...
        obj_[id] = (loadFromString ?
            Algorithm::loadFromString<HarrisLaplaceFeatureDetector>(rhs[2].toString(), objname) :
            Algorithm::load<HarrisLaplaceFeatureDetector>(rhs[2].toString(), objname));
    }
    else if (method == "save") {

...
%       (`m(x,y)`) and maximum (`M(x,y)`) `mhi` values over 3x3 neighborhood
%       of each pixel and marks the motion orientation at `(x,y)` as valid
...
%       `mhi`. Its non-zero elements mark pixels where the motion gradient
%       data is correct.

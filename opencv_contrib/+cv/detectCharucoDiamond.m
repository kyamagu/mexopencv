...
%       The real units are not necessary.
%
% ## Output
% * __diamondCorners__ output list of detected diamond corners (4 corners per
%       diamond). The order is the same than in marker corners: top left,
%       top right, bottom right and bottom left. Similar format than the
%       corners returned by cv.detectMarkers (e.g `{{[x y],..}, ..}`).
% * __diamondIds__ ids of the diamonds in `diamondCorners` (0-based). The id
%       of each diamond is in fact a vector of four integers, so each diamond
%       has 4 ids, which are the ids of the aruco markers composing the
%       diamond.
%
% ## Options

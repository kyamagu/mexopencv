classdef BinaryDescriptor < handle
    ...
    methods
        ...
        function [descriptors, keylines] = compute(this, img, keylines, varargin)
            ...
            %       `keylines{i}`. Row `j` in `descriptors` (or
        ...
        function [keylines, descriptors] = detectAndCompute(this, img, varargin)
            ...
            %       `keylines` must be accepted among the ones extracted
            %       (used when `Keylines` option is not empty).
            %       Not set by default.
            ...
            %       descriptors. So if set, detection phase will be skipped
            %       and only computation of descriptors will be executed,
            %       using the lines provided. Not set by default.
            % * __ReturnFloatDescr__ Flag (when set to true, original
            %       non-binary descriptors are returned). default false
            %
